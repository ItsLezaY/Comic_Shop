...
# internal imports 
from weekend_project.models import Product, Customer, Order, db, product_schema, products_schema
from weekend_project.forms import ProductForm
...
def shop():
    ...
    shop_stats = {
        'products': len(shop),
        'sales': sum([order.order_total for order in orders]), #order totals  = total cost of specific order
        'customers' : len(customers)
    }

    return render_template('shop.html', shop=shop, stats=shop_stats)
